...
let model: THREE.Scene | null = null;
...
const poiPosArray = [{ x: 4, y: 2, z: 0.4, frame: 1 }];
...
export default function IndexPage() {
    ...
    const loadAllTexture = (model: THREE.Object3D[]) => {
        ...
        const textures = [
            'TexturePaint_baseColor',
            'TexturePaint_emissive'
        ];
        console.log(model);
        const loadNextTexture = () => {
            const textureName = textures[loadIndex];
            textureLoader?.load('pixel_car/textures/' + textureName + '.png', function (texture) {
